...
int print_ans(FILE *fp,FILE *wr,int m,int n,int p){
	...
	fprintf(wr,"%d ",(7+((5+(17*n+2))*p+2))*m+2);
	printf("|%d ",(7+((9+(15*n+2))*p+2))*m+2);
 }
...
int simulate(char *tri,FILE *wr,int cache_size, int block_size,int cache_L2,int block_L2,int *miss_num,int *miss_L2,int situ){
	cache_size/=way;
	cache_L2/=way;
...
//			printf("\n");
	...
	int cnt=1,miss=0,missL2=0,owo=0;

	for(i=0;i<m;i++){
		for(j=0;j<p;j++){
			for(k=0;k<n;k++){
				unsigned int x[2]={l1+(i*n+k)*4,l2+(k*p+j)*4};
				owo+=2;
				for(l=0;l<2;l++){
				...
				owo++;
				if(findcache(l3+(i*p+j)*4,offset_bit,index_bit,line,cache,cnt++)){
					miss++;
					if(situ)
						if(findcache(l3+(i*p+j)*4,offset_L2,index_L2,lineL2,cacheL2,cnt)){
							missL2++;
						}
				}
			}
...
int main(int argc,char *argv[]){
	...
	char tri[4][10]={"a1xb1","a2xb2","a3xb3","a4xb4"};
	for(int i=0;i<4;i++){
	    FILE *wr = fopen("tri", "w");
		total=simulate(tri[i],wr,512,32,1,1,&miss,&missL2,0);
		...
		printf("|%d |%d ",832*miss+4*total,104*miss+4*total);
		total=simulate(tri[i],wr,128,16,4*K,128,&miss,&missL2,1);
